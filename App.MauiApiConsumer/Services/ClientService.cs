...
namespace App.MauiApiConsumer.Services
{
	public class ClientService
	{
        ...
        public async Task<List<WaterData>?> GetWaterAsync()
        {
            var httpClient = httpClientFactory.CreateClient("custom-httpclient");
            ...
            try
            {
                ...
                var response = await httpClient.GetAsync("/api/FloodApi/Top5DataSync");

                if (response.IsSuccessStatusCode)
                {
                    var boardingHouses = await response.Content.ReadFromJsonAsync<List<WaterData>>();
